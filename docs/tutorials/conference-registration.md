...
Welcome to your first hands-on tutorial for building a dynamic form! In this guide, you will build a multi-stage conference registration form from scratch.
...
1.  Open Corticon.js Studio.
2.  In the Project Explorer, right-click and select **New > Vocabulary**. Name it `ConferenceVocabulary.ecore`.
3.  Double-click your new vocabulary file to open it.
4.  You will see the standard `UI`, `Container`, `UIControl`, and `Option` entities. We need to add one more to store our registrant's information.
5.  Right-click in the vocabulary editor and select **Add New Entity**. Name it **`Registrant`**.
6.  Now, add attributes to the `Registrant` entity. Right-click the `Registrant` entity, select **Add New Attribute**, and create the following attributes one by one:
...
7.  When you are finished, your `Registrant` entity should look like this. Save your vocabulary file.
